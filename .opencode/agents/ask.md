...
- Prefer `smartly-vps-cocoindex` for semantic code discovery via the Tailscale VPS, then confirm exact evidence with `read`/`grep` when needed.
- Use `agentmemory` when prior session context, user preferences, decisions, or file history may materially improve the answer.
...
- Use `agentmemory` for cross-session context, but confirm critical code facts with local evidence.
