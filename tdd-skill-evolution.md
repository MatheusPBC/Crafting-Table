...
- [x] Review git diff, commit on `chore/platform-architecture-pack`, and push to `origin` -> Verify: `git status` is clean after the push.
...
- [x] The repo changes are committed and pushed.

## Notes
- The same skill text is intentionally mirrored across workspace, global, and repo-specific locations.
- Commit and push were completed on `chore/platform-architecture-pack`.
